function [mfD,X,Yinds] = den_spm_opm_hfc(S)
...
%--------------------------------------------------------------------------
args=[];
args.o= S.sensorDirections; % sensor orientation
args.v = S.sensorPositions; % sensor position
args.li = S.L; % harmonic order
...
%}
nChannels = size(X, 1); % Get the number of channels (1st dimension of data)
Yinds = 1:nChannels;      % Create indices for all channels
%-create ouput dataset object
%--------------------------------------------------------------------------

fprintf('Creating output dataset\n'); 
outname = fullfile(S.outputDir, 'mfD.mat') ;
%{
